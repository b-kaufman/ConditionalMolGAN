...
import math
import numpy as np
def map_nodes_to_edge_index(node_attr: torch.Tensor, edge_index: torch.Tensor, target_dim: int):
    index_map = edge_index[target_dim]
    return node_attr.index_select(-2, index_map)
...
def get_embedding(batch_size,z_dim):
    z = torch.from_numpy(np.random.normal(0, 1, size=(batch_size, z_dim))).float()
    return z
...
def get_activation_function(func_name):
    ...
    elif func_name.lower() == 'ftanh':
        return torch.tanh
    elif func_name.lower() == 'frelu':
        return torch.relu
    else:
        raise ValueError('invalid/unimplemented activation')

def get_init_function(func_name):
    if func_name.lower() == 'golort':
        return init_golort
    elif func_name.lower() == 'none':
        return None
    else:
        raise ValueError('invalid/unimplemented init')
...
"""
everything below here is from original MolGAN, identical behavior
"""
...
class MolecularMetrics(object):
    ...
    def valid_filter(mols):
        return list(filter(MolecularMetrics.valid_lambda_special, mols))
    ...
    @staticmethod
    def water_octanol_partition_coefficient_binarize(mols):
        scores = [MolecularMetrics._avoid_sanitization_error(lambda: Crippen.MolLogP(mol)) if mol is not None else None
                  for mol in mols]
        scores = np.array(list(map(lambda x: -3 if x is None else x, scores)))
        scores = np.clip(MolecularMetrics.remap(scores, -2.12178879609, 6.0429063424), 0.0, 1.0)
        scores = np.array(list(map(lambda x: 0. if x < .35 else 1., scores)))
        return scores
    ...
    @staticmethod
    def HAcceptors(mols):
        acceptors = [MolecularMetrics._avoid_sanitization_error(lambda: lip.NumHDonors(mol))
                  if mol is not None else None for mol in mols]
        return np.array(list(map(lambda x: -1 if x is None else x, acceptors)))

    @staticmethod
    def HDonors(mols):
        donors = [MolecularMetrics._avoid_sanitization_error(lambda: lip.NumHAcceptors(mol))
                  if mol is not None else None for mol in mols]
        return np.array(list(map(lambda x: -1 if x is None else x, donors)))

    @staticmethod
    def rotatableBonds(mols):
        rotate = [MolecularMetrics._avoid_sanitization_error(lambda: lip.NumRotatableBonds(mol))
                   if mol is not None else None for mol in mols]
        return np.array(list(map(lambda x: -1 if x is None else x, rotate)))

    @staticmethod
    def molWt(mols):
        weights = [MolecularMetrics._avoid_sanitization_error(lambda: HeavyAtomMolWt(mol))
                 if mol is not None else None for mol in mols]
        return np.array(list(map(lambda x: -1 if x is None else x, weights)))
...
def basic_scores(mols):
    m1 = {'valid score': MolecularMetrics.valid_total_score(mols) * 100,
          'unique score': MolecularMetrics.unique_total_score(mols) * 100}
    return m1

def conditional_basic_scores(mols):
    m1 = {'valid score': MolecularMetrics.valid_total_score(mols) * 100,
          'unique score': MolecularMetrics.unique_total_score(mols) * 100,
          'LogP': np.mean(MolecularMetrics.water_octanol_partition_coefficient_scores(mols))}
    return m1
